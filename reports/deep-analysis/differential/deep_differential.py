...
def main():
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('stitch',type=Path);ap.add_argument('--out',required=True,type=Path);args=ap.parse_args()
    ...
    for i,(a,b) in enumerate([('position-01','position-02'),('position-01','position-03'),('position-02','position-03')]):
        ...
        index=np.arange(0,len(f),10);rows=diagnose(f[index],logs[0][index],logs[1][index])
