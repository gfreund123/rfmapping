...
def stitch(rows,spacing_hz=50000):
    ...
    first=np.ceil((rows[0]['center']-1600000)/spacing_hz)*spacing_hz
    last=np.floor((rows[-1]['center']+1600000)/spacing_hz)*spacing_hz
    f=np.arange(first,last+spacing_hz/2,spacing_hz)
    total=np.zeros(len(f),complex);weights=np.zeros(len(f))
    for i,r in enumerate(rows):
        u=(f-r['center'])/1e6;mask=(abs(u)<=1.6)&(abs(u)>=.12)
