...
def calibrate_overlap_baseband(rows,phase_degree=5,amplitude_degree=6):
    """Estimate shared filter curvature assuming common fractional sample timing.

    Ratios at the same RF frequency cancel a stationary physical channel. Each
    overlap's constant phase/gain is removed; per-window linear phase is NOT free.
    Validate this timing assumption on independent sweeps and known drift controls.
    """
    phase_design=[];amplitude_design=[];phase_values=[];amplitude_values=[]
    for i,a in enumerate(rows[:-1]):
        b=rows[i+1];f=a['f']+a['center'];ub=(f-b['center'])/1e6;ua=a['f']/1e6
        mask=(abs(ua)<1.65)&(abs(ub)<1.65)&(abs(ub)>.12)
        f=f[mask];ua=ua[mask];ub=ub[mask];x=a['h'][mask];y=interpolate(b,f)
        phase=np.unwrap(np.angle(x*np.conj(y)));mag=np.log(abs(x/y))
        dp=np.column_stack([ua**k-ub**k for k in range(2,phase_degree+1)])
        da=np.column_stack([ua**k-ub**k for k in range(2,amplitude_degree+1)])
        noise=np.mean(a['v'][mask])/np.mean(abs(x)**2)+np.mean(b['v'])/np.mean(abs(y)**2)
        weight=1/np.sqrt(max(noise,1e-8))
        phase_design.append((dp-dp.mean(axis=0))*weight);amplitude_design.append((da-da.mean(axis=0))*weight)
        phase_values.append((phase-phase.mean())*weight);amplitude_values.append((mag-mag.mean())*weight)
    pc=np.r_[0,0,np.linalg.lstsq(np.vstack(phase_design),np.concatenate(phase_values),rcond=None)[0]]
    ac=np.r_[0,0,np.linalg.lstsq(np.vstack(amplitude_design),np.concatenate(amplitude_values),rcond=None)[0]]
    return {'phase_coefficients_rad_per_mhz_power':pc.tolist(),'log_amplitude_coefficients_per_mhz_power':ac.tolist(),
        'training_windows':len(rows),'calibration_method':'Adjacent overlap ratios, constant gain/phase removed, common fractional timing assumed.',
        'assumption':'Within each training sweep the physical channel is stationary and integer alignment leaves a common fractional sample timing. Independent timing errors or scene changes can bias the estimated response.'}
...
def stitch(rows,spacing_hz=50000,timing='free'):
    n=len(rows);edges=[]
    for i in range(n):
    ...
    for i in range(n-1):
        ...
        initial_beta[i+1]=initial_beta[i]-e['beta'] if timing=='free' else 0
    ...
    if timing=='fixed':keep[n:]=False
    elif timing!='free':raise ValueError('Unknown timing model')
...
def delay_profile(f,h,nfft=65536,window='hann'):
    if window=='hann':w=np.hanning(len(f))
    elif window=='blackman':w=np.blackman(len(f))
    elif window=='rectangular':w=np.ones(len(f))
    else:raise ValueError('Unknown spectral window')
