...
from deep_stitch import prepare,estimate_baseband,corrected_rows,stitch,align_responses,delay_profile
...
def main():
    ...
    training=prepared[0][1]['forward'];model=estimate_baseband(training)
    result={'schema':'rfmapping.deep-stitch/v1','created_utc':utc(),'hardware_access':False,
        'degree':args.degree,'quarter':args.quarter,'training_run':prepared[0][0]['run_id'],'training_sweep':'forward',
    ...
    for position,(r,sweeps) in enumerate(prepared):
        ...
        for direction,rows in sweeps.items():
            s=stitch(corrected_rows(rows,model));solutions[direction]=s
