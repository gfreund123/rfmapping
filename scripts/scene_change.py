...
Commands B and C start the corresponding stage after external coordination.
Record actual human replies and cue timing separately; stdin is not proof of pose
or occupancy. Input timeout or EOF stops the session and restores settings.
"""
...
def await_command(expected,timeout=300):
    ...
    return {'command':reply,'received_utc':utc(),'meaning':'Controller requested this stage; see operator-events for human replies and cue timing.'}
...
def main():
    ...
    try:
        ...
        for phase in ('A','B','C'):
            ...
            # capture_tile rewrites the same requested LO/gain before each stage;
            # this is not a measurement of calibration-state continuity.
            for b in range(8):
                ...
                save_json(path.with_suffix('.sigmf-meta'),{'global':{'core:datatype':'ci16_le','core:version':'1.2.5','core:sample_rate':int(r.rx_channels[0].attrs['sampling_frequency'].value),'core:description':'Scene A/B/A coded pilot. Labels follow controller commands; consult operator-events for actual human observations. No calibrated range.','rfmapping:phase':phase},'captures':[{'core:sample_start':0,'core:frequency':int(r.rxlo.attrs['frequency'].value)}],'annotations':[]})
