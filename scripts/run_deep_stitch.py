...
def main():
    ...
    ap.add_argument('--model',type=Path,help='Use the frozen baseband_model in an earlier stitch.json')
    ...
    training=prepared[0][1]['forward']
    model=json.loads(args.model.read_text())['baseband_model'] if args.model else (calibrate_overlap_baseband if args.calibration=='overlap' else estimate_baseband)(training)
    result={'schema':'rfmapping.deep-stitch/v1','created_utc':utc(),'hardware_access':False,
        ...
        'frozen_model_source':{'path':args.model.resolve().relative_to(ROOT).as_posix(),'sha256_lf_utf8':hashlib.sha256(args.model.read_text().encode()).hexdigest()} if args.model else None,
