...
def blocked(f,forward,reverse,degree,delays=DELAYS):
    fold=(np.floor((f-f.min())/8000000).astype(int))%3;rows=[];base_error=echo_error=0;count=0
    ...
    return {'prediction_squared_error_reduction':1-echo_error/max(base_error,1e-30),
        'baseline_complex_log_prediction_rmse':float(np.sqrt(base_error/count)),
        'echo_complex_log_prediction_rmse':float(np.sqrt(echo_error/count)),
        'fold_preferred_delays_ns':[x['delay_ns'] for x in rows],'folds':rows}
