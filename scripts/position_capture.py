...
class Session:
    def __init__(self,args):
        if not args.position_id.replace('-','').isalnum():
            raise ValueError('Use letters, numbers and hyphens for position ID')
        ...
        self.survey_from=getattr(args,'survey_from',None)
        if self.survey_from:
            previous=json.loads(self.survey_from.read_text())
            inherited=[dict(x,origin_run_id=previous['run_id']) for x in previous['records']
                       if x['id'].startswith(('ambient24_','survey58_'))]
            if previous['plan']['position_id']!=args.position_id or len(inherited)!=101:
                raise ValueError('Survey must contain all 27 + 74 receive captures from this position ID')
            if any(x['activity'].get('usable_edge_hz')!=1450000 for x in inherited):
                raise ValueError('Survey uses a different receive guard profile')
            age=datetime.now(timezone.utc)-datetime.fromisoformat(previous['ended_utc'])
            if age.total_seconds()>900:raise ValueError('Inherited survey older than 15 minutes; resurvey')
            self.result['inherited_survey']={'run_id':previous['run_id'],
                'results_relative_path':self.survey_from.resolve().relative_to(ROOT).as_posix(),
                'results_sha256_lf_utf8':hashlib.sha256(self.survey_from.read_text().encode()).hexdigest(),
                'assumption':'No device movement was requested between these attempts; fresh guards still precede every RF burst.'}
            self.result['records']=inherited
            self.result['hot_intervals_hz']=previous['hot_intervals_hz']
            self.bytes=sum(x['raw_bytes'] for x in inherited)
        self.save()
    ...
    def save(self):
        ...
        # Compact live snapshots avoid repeatedly pretty-printing a multi-MB
        # manifest. Atomic replacement also prevents readers seeing partial JSON.
        finished='ended_utc' in self.result
        text=json.dumps(self.result,allow_nan=False,indent=2 if finished else None,
                        separators=None if finished else (',',':'))+'\n'
        pending=self.public/'results.pending'
        pending.write_text(text,encoding='utf-8',newline='\n')
        # Windows sync/indexing readers may briefly deny rename. Keep the
        # previous complete snapshot and retry the same pending bytes; if the
        # lock persists, stop with both files retained rather than truncate it.
        for attempt in range(21):
            try:
                pending.replace(self.public/'results.json')
                break
    ...
    def open_radio(self):
        ...
        # Check the installed libiio buffer API before any long survey. No push
        # and no unmute here: only inspect and fill the host-side TX allocation.
        for ch in self.txchannels:ch.enabled=True
        testbuf=None
        try:
            tx,_=self.waveforms['overlap-3p6MHz']
            testbuf=r.iio.Buffer(r.dds,len(tx),True)
            payload=bytearray(tx.tobytes())
            if testbuf.step!=4 or len(testbuf)!=len(payload) or testbuf.write(payload)!=len(payload):
                raise RuntimeError('TX allocation/write layout check failed while muted')
        finally:
            if testbuf is not None:testbuf.cancel();del testbuf
            r.mute()
        self.result['muted_tx_buffer_api_check']='passed without push or unmute'
        self.result['rx_tracking_modes']={k:r.rx.attrs[k].value for k in
            ('quadrature_tracking_en','rf_dc_offset_tracking_en','bb_dc_offset_tracking_en') if k in r.rx.attrs}
        self.result['clock_and_calibration']={k:r.phy.attrs[k].value for k in
            ('rx_path_rates','tx_path_rates','calib_mode','trx_rate_governor') if k in r.phy.attrs}
        self.result['tx_sample_rate_hz']=int(r.tx.attrs['sampling_frequency'].value)
        private=self.local/'context.xml'
        private.write_text(r.ctx.xml,encoding='utf-8')
        self.result['private_context']={'raw_relative_path':private.relative_to(ROOT).as_posix(),
            'sha256':hashlib.sha256(private.read_bytes()).hexdigest(),
            'note':'Ignored local context includes device identity; not published.'}
    ...
    def transmit(self,name,waveform,attenuation=45):
        ...
        try:
            ...
            payload=bytearray(tx.tobytes())
            if buf.step!=4 or len(buf)!=len(payload) or buf.write(payload)!=len(payload):raise RuntimeError('TX buffer mismatch')
...
def main():
    ...
    ap.add_argument('--survey-from',type=Path,help='Reuse a complete recent same-position passive survey after an interrupted attempt; fresh RF guards remain mandatory.')
