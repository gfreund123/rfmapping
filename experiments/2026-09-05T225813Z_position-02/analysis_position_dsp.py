...
def averaged_pilot_evidence(summary,off_reference=0.):
    """Averaged-response evidence; does not promise precise individual bin phase.

The earlier rho>0.15 flag is conservative per-period screening. This additional
rule also uses coherent averaging noise and phase residuals, tested against pure
noise and a known weak pilot. Retain the original flag in the capture record.
"""
    return bool(summary['median_correlation']>max(.06,2.5*off_reference)
                and summary['estimator_noise_to_signal_ratio']<.25
                and summary['phase_residual_rms_deg']<15)
