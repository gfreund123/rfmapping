...
class Session:
    ...
    def receive(self,name,blocks=4):
        ...
        rec['activity']=activity(iq,rec['settings']['stream_sample_rate_hz'])
    ...
    def probe(self,name,lo,waveform='overlap-3p6MHz',attenuation=45,retune=True):
        ...
        if retune:self.tune(lo,bandwidth=bw)
        guard,_=self.receive(name+'_guard',4);job['guard_id']=guard['id']
        if not guard['quiet_observed']:
            job['status']='skipped-fresh-activity'
            actual=guard['settings']['rx_lo_hz']
            for offset in guard['activity']['hot_offsets_hz']:
    ...
    def anchors(self,label):
        for lo in ANCHORS:
            for i in range(3):self.probe(f'{label}_{lo}_repeat{i+1}',lo,retune=(i==0))
            if label in ('start','end'):
                self.probe(f'{label}_{lo}_atten48',lo,attenuation=48,retune=False)
    ...
    def run(self):
        ...
        for i,lo in enumerate(range(5727000000,5873000001,2000000)):
            self.tune(lo);rec,_=self.receive(f'survey58_{lo}',8)
        ...
        for i in range(3):self.probe(f'legacy_{i+1}',ANCHORS[0],'legacy-1p8MHz',retune=(i==0))
        self.anchors('start')
