...
def spectral_summary(iq, fs):
    ...
    frames=z.reshape(-1,nfft)
    w=np.hanning(nfft)
    p=np.abs(np.fft.fft(frames*w,axis=1))**2/(fs*np.sum(w*w)*2048**2)
    # Four-window power average (1.6384 ms at 5 MS/s) before temporal maximum.
    p=p[:len(p)//4*4].reshape(-1,4,nfft).mean(axis=1)
    p=np.fft.fftshift(p,axes=1)
    f=np.fft.fftshift(np.fft.fftfreq(nfft,1/fs))
    bins=[]
    for center in np.arange(-1475000,1500000,50000):
        mask=(f>=center-25000)&(f<center+25000)
        power=p[:,mask].mean(axis=1)
